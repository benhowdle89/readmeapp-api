...
module.exports = class Twitter {
  ...
  fetchTweets(accessToken, accessTokenSecret, userId, latestId) {
    ...
    return new Promise((resolve, reject) => {
      oauth.get(timelineUrl, accessToken, accessTokenSecret, (error, data) => {
        if (error) return reject({ errors: error });
        this.getMuted(accessToken, accessTokenSecret).then(
          ({ data: muted }) => {
            const tweets = this.filterSelf(
              this.filterQuotes(
                this.filterRTs(
                  this.filterReplies(this.filterMuted(JSON.parse(data), muted))
                )
              ),
              userId
            );
